...
def experiments_sim(n, fr, uti, hr, inputfile, simmode, distribution, threshold = 0):
    ...
    try:
        filename = 'outputs/sim'+str(n)+'_'+str(uti)+'_'+str(tasksets_amount)+'_'+str(hr)+'_Sim'
        SIM = np.load(filename+'.npy')
        filename = 'outputs/simt'+str(n)+'_'+str(uti)+'_'+str(tasksets_amount)+'_'+str(hr)+'_Sim'
    ...
    if Outputs is False:
        ...
        for idx, tasks in enumerate(tasksets):
            ...
            resultfilename = 'result_hardperc_' + str(hr) + 'faultprob_' + str(fr) + 'simode_' + str(simmode) + 'th_' + str(threshold) + '.txt'
    ...
    print "Result for Safe Rate-"+str(fr)+"_uti-"+str(uti)
