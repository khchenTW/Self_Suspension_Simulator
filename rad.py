...
def alpha_t(t, itask, i):
    ...
    segmentslist = [itask['firstSub'], itask['secondSub']]
    sumT = 0
    numcseg = len(segmentslist)
    while 1:

        if l % numcseg != numcseg - 1:
            sumT += segmentslist[l % numcseg] + segmentslist[l % numcseg]
        else:
            if l <= numcseg:
                sumT += segmentslist[numcseg - 1] + itask["period"] - itask["period"]
            else:
                sumT += segmentslist[numcseg - 1] + (
                            itask["period"] - (itask["execution"] + itask["suspTime"] * 1))
        if sumT >= t:
            break
        else:
            l += 1
...
def ssRTA_withLocal(Cn, HPTasks, Tn):
    R = 0
    while True:

        dm = 0
        for itask in HPTasks:
            dm_local = (itask['local'] + itask['execution']) * math.ceil((R) / itask['period'])
            dm_normal = MRBF(R, itask)
            dm += max([dm_normal,dm_local])
...
def SUMTest(itask, HPTasks):
    R = 0
    segmentlist = [itask['firstSub'], itask['secondSub']]
    Tn = itask['period']
    if itask['suspTime'] == 0:
        R = ssRTA(itask['execution'], HPTasks, Tn)
    else:
        for iseg in segmentlist:
            R += ssRTA(iseg, HPTasks, Tn)
            if R > Tn:
                return False

    if R + itask['suspTime'] > Tn:
        return False
    else:
        return True

def SUMTest_responsetime(itask, HPTasks):
    R = 0
    segmentlist = [itask['firstSub'], itask['secondSub']]
    Tn = itask['period']
    if itask['suspTime'] == 0:
...
def SUMTest_responsetime_firstsegment(itask, HPTasks):
    R = 0

    R = ssRTA(itask['firstSub'], HPTasks, itask['period'])
...
def SUMTest_withlocal(itask, HPTasks):
    R = 0
    segmentlist = [itask['firstSub'], itask['secondSub']]
    Tn = itask['period']
    if itask['suspTime'] == 0:
        R = ssRTA_withLocal(itask['execution'], HPTasks, Tn)
    else:
        for iseg in segmentlist:
            R += ssRTA_withLocal(iseg, HPTasks, Tn)
            if R > Tn:
                return False
...
def SUMTestFirst(itask, HPTasks):
    R = 0
    Tn = itask['period']
    R = ssRTA(itask['firstSub'], HPTasks, Tn)
...
def segTest(Cn, Sn, Tn, HPTasks): # returns response time or zero
    R = 0
...
def segTest_responsetime(Cn, Sn, Tn, HPTasks): # returns response time
    R = 0
...
def segTestwithLocal(local, Cn, Sn, Tn, HPTasks): # returns response time or zero
    R = 0
    ...
    while True:
        ...
        dm = 0
        for itask in HPTasks:
            dm_local = (itask['execution'] + itask['local']) * math.ceil((R) / itask['period'])
            dm_normal = MRBF(R, itask)
            dm += max([dm_local,dm_normal])
...
def segTestFirst(firstSub, Sn, Tn, HPTasks): # returns response time or zero
    R = 0
...
def Burst_HP(Cn, Sn, Tn, HPTasks):
    ...
    for i in range(len(sortedTasksAlpha)):
        ...
        sumL += (sortedTasksAlpha[i]['alpha'] + 1) * (
                    sortedTasksAlpha[i]['execution'] / sortedTasksAlpha[i]['period']) / prod
    if (Cn + Sn) / Tn <= 1 - sumL:
        return True
    else:
        return False
...
def scair_dm(tasks):
    ...
    for i in range(len(sortedTasks)):
        ...
        # if primeTasks:
        respTimeSeg = segTest(Cn, Sn, Tn, primeTasks)
        respTimeSum = SUMTest(sortedTasks[i], primeTasks)
        if respTimeSeg == 0 and respTimeSum == 0:
            return False
        else:
            accept = True
...
def scair_dm_responsetimefirstsegment(currentTask, hpTasks): # returns worst case response time for first segment of a task
    # shortest period first
    sortedTasks = sorted(hpTasks, key=lambda item: item['period'])

    responsetime = currentTask['execution']

    if hpTasks and len(hpTasks) != 0:
        respTimeSeg = segTest_responsetime(currentTask['firstSub'], 0, currentTask['period'], hpTasks)
        respTimeSum = SUMTest_responsetime_firstsegment(currentTask, hpTasks)

        if respTimeSeg > respTimeSum:
            responsetime = respTimeSeg

        else:
            responsetime = respTimeSum

    return responsetime


def transientIAnalysis(tasks):
    sortedTasks = sorted(tasks, key=lambda item: item['period'])
    for i in range(len(sortedTasks)):
        first = 0
        ...
        Tn = sortedTasks[i]['period']
        Cn = sortedTasks[i]['execution']
        firstSub = sortedTasks[i]['firstSub']
        secondSub = sortedTasks[i]['secondSub']
        deadline = sortedTasks[i]['deadline']
        suspTime = sortedTasks[i]['suspTime']
        local = sortedTasks[i]['local']
        # tasks with higher prio than current task
        primeTasks = []
        if i != 0:
            primeTasks = sortedTasks[:i]
        ...
        respTimeSeg = segTestFirst(firstSub, 0, Tn, primeTasks)
        respTimeSum = SUMTestFirst(sortedTasks[i], primeTasks)
        first = 0
        if currenttask['type'] == 'hard' and (respTimeSeg == 0 and respTimeSum == 0):
            return 0
        else:
            first = min([respTimeSeg,respTimeSum])

        # TDA
        normalTDArespTime = TDA.TDAhard(currenttask,primeTasks)
        transientTDARespTime = TDA.TDAhard_transient(currenttask,primeTasks, first)
        second = max([normalTDArespTime, transientTDARespTime])
        if currenttask['type'] == 'hard' and second > deadline:
            return 0

        totalresponsetime = first + suspTime + second
        if currenttask['type'] == 'hard' and totalresponsetime > deadline:
            return 0
        else:
            return totalresponsetime


def transientIIAnalysis(tasks):
    # shortest period first
    sortedTasks = sorted(tasks, key=lambda item: item['period'])
    ...
    for i in range(len(sortedTasks)):
        ...
        # if primeTasks:
        respTimeSeg = segTestwithLocal(local,Cn, Sn, Tn, primeTasks)
        respTimeSum = SUMTest_withlocal(sortedTasks[i], primeTasks)
        if currenttask['type'] == 'hard' and (respTimeSeg == 0 and respTimeSum == 0):
            return False
        else:
            accept = True
